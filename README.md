...
A `Node.js` Windows command-line tool used for changing a PDF document's page dimensions (measured in pts) while preserving the top, left, and right margins proportions (i.e., only the bottom margin proportion may change after the resizing) and removing the output PDF document's metadata afterwards.
